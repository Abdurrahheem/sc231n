...
def affine_forward(x, w, b):
    ...
    xline = x.reshape(x.shape[0],-1)
    out   = xline.dot(w) + b
#     print(out.shape[1] == b.shape[0])
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (mode, x, xav, std, gamma, x_norm, out, var, eps, mu)
...
#     xav = x - mu
#     var = np.mean(xav**2, axis=1)[...,None]
#     std = np.sqrt(var + eps)
#     x_norm = xav/std
#     out = gamma * x_norm + beta
    
#     cache = (xav, std, gamma, x_norm, var, eps)
...
def layernorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
#     xav, std, gamma, x_norm, var, eps = cache
...
#     dstdover1 = np.sum(dx_norm*xav, axis=1)[...,None]
#     dstd = -1./std**2 * dstdover1
#     print(dstd.shape)
...
#     dmu = -1. * np.sum(dxav1 + dxav2, axis = 0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    if pad != 0:
        xp = np.pad(x, pad_width=[(0,),(0,),(pad,),(pad,)],  mode='constant', constant_values=0)
    
    for n in range(N):
        for f in range(F):
            for hh in range(H1):
                for ww in range(W1):
                    out[n,f,hh,ww]= np.sum(xp[n, :, hh*stride : hh*stride+HH, ww*stride : ww*stride+WW] * w[f,:,:,:]) + b[f]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H1):
                for w in range(W1):
                    win =  x[n,c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width ] 
                    zeros = win == np.max(win)
                    dx[n,c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width] = dout[n,c,h,w]*zeros
